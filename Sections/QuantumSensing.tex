...
Quantum sensors have a higher \index{sensitivity}{sensitivity} within a nanoscale or microscale sampling volume compared to a fully classical counterpart which would require higher field densities or higher volume interrogation to be effective. 
...
Spin defects are mostly paramagnetic and radiative point defects (or colour centres). \index{colour centre}{Colour centre}s possessing a 
non-zero electron spin are excellent candidates for optical spin
quantum bits (\index{qubit}{qubit}s) \cite{Castelletto_2024}.
